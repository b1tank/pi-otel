  ...
  return Object.fromEntries(
    value.split(",").flatMap((entry) => {
      ...
      try {
        return [
          [
            decode ? decodeURIComponent(key) : key,
            decode ? decodeURIComponent(raw) : raw,
          ],
        ];
      } catch {
        return [];
...
export function resolveConfig(
  ...
  const enabled =
    truthy(env.PI_OTEL_ENABLED) || Boolean(env.OTEL_EXPORTER_OTLP_ENDPOINT);
  return {
    enabled,
    captureContent:
      truthy(env.OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT) ||
      truthy(env.PI_OTEL_CAPTURE_CONTENT),
