...
describe("Telemetry", () => {
  it("exports traces, metrics, and logs and honors content capture", async () => {
    ...
    const operation = telemetry.start("chat test", SpanKind.CLIENT, { "gen_ai.operation.name": "chat" })
    telemetry.event("pi.test", { content: telemetry.content("sensitive prompt") }, operation)
    ...
    expect(telemetry.content("1234567890123456789012345")).toContain("TRUNCATED")
  })
})
